...
-(void)startWatchingMediaKeys;{
	...
	_eventPort = CGEventTapCreate(kCGSessionEventTap,
								  ...
								  self);
	if (_eventPort == NULL) {
		NSLog(@"[SPMediaKeyTap] ERROR: Failed to create event tap. This app needs Accessibility permissions.");
		NSLog(@"[SPMediaKeyTap] To fix: Open System Settings > Privacy & Security > Accessibility");
		NSLog(@"[SPMediaKeyTap] Then add PlexMusicDaemon to the list and restart the daemon");
		return;
	}

    _eventPortSource = CFMachPortCreateRunLoopSource(kCFAllocatorSystemDefault, _eventPort, 0);
	if (_eventPortSource == NULL) {
		NSLog(@"[SPMediaKeyTap] ERROR: Failed to create run loop source");
		return;
	}
